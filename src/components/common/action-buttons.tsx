...
type Props = {
  ...
  enableDisableCourseButton?: boolean;
...
const ActionButtons = ({
  ...
  function handleResetUserPassword() {
    openModal('REST_USER_PASSWORD_VIEW', id)
  }
  ...
  function handleCourseStatus(isCourseActive: boolean) {
    if (isCourseActive === true) {
      openModal('COURSE_DISABLE_VIEW', {id, type: 'enable'});
    } else {
      openModal('COURSE_DISABLE_VIEW', {id, type: 'disable'});
    }
  }
  ...
  return (
    <div className="inline-flex items-center w-auto gap-3">
      ...
      {enableDisableCourseButton &&
        (!isCourseActive ? (
          <button
            onClick={() => handleCourseStatus(true)}
            className="transition duration-200 text-accent hover:text-accent-hover focus:outline-none"
          ...
          <button
            onClick={() => handleCourseStatus(false)}
            className="text-red-500 transition duration-200 hover:text-red-600 focus:outline-none"
            title='Enable course ?'
          >
            <CloseFillIcon width={17} />
