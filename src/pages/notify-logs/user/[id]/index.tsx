...
import { NotifyType, SortOrder } from '@/types';
import ShopLayout from '@/components/layouts/shop';
import PageHeading from '@/components/common/page-heading';
import NotFound from '@/components/ui/not-found';
import StudentLayout from '@/components/layouts/student';
import { useNotificationReadMutation, useNotificationsQuery } from '@/data/notification';
...
const NotifyLogItem = ({ item }: any) => {
  ...
  switch (item?.notify_type) {
    ...
    case NotifyType.COURSE_NOTICE:
      notifyLogText = 'text-sent-new-course-notice';
      break;
  }
  ...
  const handleClickOnNotification = (value: any) => {
    ...
    switch (value?.notify_type) {
      ...
      case 'store_notice':
        ...
        redirectTo = Routes?.storeNotice?.details(value?.id);

        break;

      case NotifyType.COURSE_NOTICE:
        //@ts-ignore
        redirectTo = Routes?.courseNotice?.details(value?.id);
...
NotifyLogsPageForVendor.Layout = StudentLayout;
